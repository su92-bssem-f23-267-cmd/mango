...
import { getVarietyById, updateVariety } from '@/actions/varietyActions'
...
export default function EditVarietyPage() {
  ...
  useEffect(() => {
    async function loadVariety() {
      const res = await getVarietyById(varietyId)
      if (res.error) {
        toast.error(res.error)
        router.push('/admin/varieties')
      } else if (res.variety) {
        setName(res.variety.name)
        setDescription(res.variety.description || '')
        setIsActive(res.variety.isActive)
      }
      setLoading(false)
    }
    loadVariety()
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await updateVariety(varietyId, formData)
    if (res.error) {
      setError(res.error)
      toast.error(res.error)
    } else {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-black text-primary tracking-tight">✏️ Edit Variety</h1>
          <p className="text-xs text-muted-foreground mt-0.5">Update the variety classification details.</p>
        </div>
      </div>
